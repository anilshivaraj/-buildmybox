...
// Mock out the pubsubebikes lib and use these mocks to verify how functions were called
jest.mock('c/pubsubebikes', () => {
    return {
        fireEvent: jest.fn(),
...
describe('c-product-tile-list', () => {
    ...
    describe('getProduct @wire emits records', () => {
        it('renders paginatorebikes with correct item counts', () => {
            ...
            return Promise.resolve().then(() => {
                const paginatorebikes = element.shadowRoot.querySelector(
                    'c-paginatorebikes'
                );
                expect(paginatorebikes).not.toBeNull();

                // paginatorebikes text will look something like: "12 items • page 1 of 2"
                ...
                expect(paginatorebikes.shadowRoot.textContent).toMatch(regex);
            });
        });
        ...
        it('increments/decrements page number when "next" and "previous" events fired', () => {
            ...
            return Promise.resolve()
                .then(() => {
                    ...
                    paginatorebikes.dispatchEvent(new CustomEvent('next'));
                })
                .then(() => {
                    ...
                    expect(paginatorebikes.shadowRoot.textContent).toMatch(
                        regex
                    );

                    paginatorebikes.dispatchEvent(new CustomEvent('previous'));
                })
                .then(() => {
                    const paginatorebikes = element.shadowRoot.querySelector(
                        'c-paginatorebikes'
    ...
    describe('getProduct @wire emits empty list of records', () => {
        it('does not render paginatorebikes', () => {
            const element = createElement('c-product-tile-list', {
                is: ProductTileList
            });
            document.body.appendChild(element);
            ...
            return Promise.resolve().then(() => {
                ...
                expect(paginatorebikes).toBeNull();
            });
        });
